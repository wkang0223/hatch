...
impl IdleDetector {
    ...
    pub fn poll(&mut self) -> Option<IdleState> {
        ...
        // HATCH_FORCE_AVAILABLE=1 skips screen-lock check (dev/testing only)
        let force       = std::env::var("HATCH_FORCE_AVAILABLE").map(|v| v == "1").unwrap_or(false);
