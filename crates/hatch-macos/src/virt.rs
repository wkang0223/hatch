...
//!   - Shared directory mount: /tmp/hatch/<job_id>/ → /job in VM
//!
...
impl VmConfig {
    ...
    pub fn for_job(job_id: &str, job_dir: PathBuf, ram_limit_gb: u32) -> Self {
        Self {
            ...
            job_dir,
            base_image: PathBuf::from("/var/hatch/base-linux-arm64.img"),
...
/// Install the nm-vm-helper Swift binary from the Hatch bundle.
...
pub fn install_vm_helper() -> Result<()> {
    ...
    let url = "https://releases.hatch.io/tools/nm-vm-helper-arm64";
...
pub fn ensure_base_image() -> Result<PathBuf> {
    let img_path = PathBuf::from("/var/hatch/base-linux-arm64.img");
    ...
    std::fs::create_dir_all("/var/hatch")?;

    let url = "https://releases.hatch.io/images/hatch-base-arm64-v1.img.gz";
    info!("Downloading base Linux image (this may take a few minutes)…");

    let status = Command::new("curl")
        .args([
            "-fsSL", "--progress-bar",
            "-o", "/var/hatch/base-linux-arm64.img.gz",
            url,
        ])
        .status()
    ...
    let status = Command::new("gunzip")
        .arg("/var/hatch/base-linux-arm64.img.gz")
        .status()
