//! ArbitrumOracle — on-chain settlement for Hatch Phase 3.
//!
...
//! |-----------------------|-------------------------------------------------|
//! | `HATCH_ARBITRUM_RPC`     | HTTPS RPC endpoint (Alchemy / Infura / Ankr)    |
//! | `HATCH_ORACLE_PK`        | Oracle private key hex, 64 chars (no 0x prefix) |
//! | `HATCH_ESCROW_ADDRESS`   | Deployed `Escrow.sol` address (0x…)             |
//!
//! If `HATCH_ESCROW_ADDRESS` is absent the oracle is disabled (all calls return
...
fn htc_to_wei_u128(nmc: f64) -> Result<u128> {
    if nmc < 0.0 {
        bail!("htc_to_wei: negative amount");
    }
...
impl ArbitrumOracle {
    ...
    pub fn from_env() -> Self {
        let escrow_addr = std::env::var("HATCH_ESCROW_ADDRESS").unwrap_or_default();
        if escrow_addr.is_empty() {
            info!("HATCH_ESCROW_ADDRESS not set — on-chain settlement disabled");
            return Self::disabled();
        }
        let rpc_url = std::env::var("HATCH_ARBITRUM_RPC")
            .unwrap_or_else(|_| "https://sepolia-rollup.arbitrum.io/rpc".to_string());
        let oracle_pk = std::env::var("HATCH_ORACLE_PK").unwrap_or_default();
        if oracle_pk.len() != 64 {
            warn!("HATCH_ORACLE_PK missing or invalid length — on-chain disabled");
            return Self::disabled();
        }
    ...
    fn signing_key(&self) -> Result<SigningKey> {
        let bytes = hex::decode(&self.oracle_pk_hex)
            .context("Invalid HATCH_ORACLE_PK hex")?;
    ...
    pub async fn release_escrow(
        ...
        let cost_wei = htc_to_wei_u128(actual_cost_htc)?;
...
mod tests {
    ...
    #[test]
    fn test_htc_to_wei_whole() {
        assert_eq!(htc_to_wei_u128(1.0).unwrap(), 10u128.pow(18));
    }

    #[test]
    fn test_htc_to_wei_fraction() {
        let wei = htc_to_wei_u128(0.5).unwrap();
    ...
    fn test_disabled_oracle() {
        std::env::remove_var("HATCH_ESCROW_ADDRESS");
