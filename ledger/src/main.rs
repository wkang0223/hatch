//! hatch-ledger — off-chain credit accounting (Phase 1) + on-chain
...
#[derive(Parser, Debug)]
#[command(name = "hatch-ledger", version)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    info!(version = env!("CARGO_PKG_VERSION"), "hatch-ledger starting");
    ...
    // Phase 3: on-chain oracle (no-op when HATCH_ESCROW_ADDRESS is absent)
...
async fn health() -> Json<serde_json::Value> {
    Json(serde_json::json!({ "status": "ok", "service": "hatch-ledger" }))
}
...
async fn on_chain_release(
    ...
    if body.actual_cost_htc < 0.0 {
        return Err(StatusCode::BAD_REQUEST);
    }

    match state.oracle.release_escrow(&body.job_id, body.actual_cost_htc).await {
        Ok(Some(tx_hash)) => {
