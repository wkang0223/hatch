...
async fn install(runtimes: String, ctx: &ClientContext) -> Result<()> {
    ...
    // Create hatch_worker user
    println!("\n{}", "Creating isolated worker user...".bold());
    let user_exists = Command::new("id").arg("hatch_worker").status()?.success();
    if !user_exists {
        let status = Command::new("sudo")
            .args(["dscl", ".", "-create", "/Users/hatch_worker"])
            .status()?;
        if status.success() {
            Command::new("sudo")
                .args(["dscl", ".", "-create", "/Users/hatch_worker", "UserShell", "/usr/bin/false"])
                .status()?;
            println!("  {} Created hatch_worker user", "✓".green());
        }
    } else {
        println!("  {} hatch_worker user already exists", "✓".green());
    }
    ...
    // Create /tmp/hatch directory
    std::fs::create_dir_all("/tmp/hatch")?;
    println!("\n{} Working directory created: /tmp/hatch", "✓".green());
...
async fn configure(
    ...
    let mut cfg: hatch_common::config::AgentConfig = if content.is_empty() {
        hatch_common::config::AgentConfig::default()
    } else {
    ...
    if let Some(p) = floor_price    { cfg.floor_price_htc_per_hour = p; }
...
/// - Loads or generates an Ed25519 keypair at ~/.config/hatch/identity.key
...
pub async fn ensure_account(ctx: &ClientContext) -> Result<String> {
    use hatch_crypto::keys::HatchKeypair;
    ...
    let keypair = if key_path.exists() {
        HatchKeypair::load_from_file(&key_path).context("Loading keypair")?
    } else {
        let kp = HatchKeypair::generate();
