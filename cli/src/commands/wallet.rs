...
async fn show_balance(ctx: &ClientContext) -> Result<()> {
    ...
    if ctx.output_json {
        println!("{}", serde_json::to_string_pretty(&serde_json::json!({
            ...
            "available_htc": bal.available_htc,
            "escrowed_htc": bal.escrowed_htc,
            "total_earned_htc": bal.total_earned_htc,
            "total_spent_htc": bal.total_spent_htc,
        }))?);
        return Ok(());
    }
    ...
    println!("  Available:    {} HC", format!("{:.4}", bal.available_htc).green().bold());
    if bal.escrowed_htc > 0.0 {
        println!("  In escrow:    {} HC  (reserved for active jobs)", format!("{:.4}", bal.escrowed_htc).yellow());
    }
    println!("─────────────────────────────────────────");
    println!("  Total earned: {} HC", format!("{:.4}", bal.total_earned_htc).blue());
    println!("  Total spent:  {} HC", format!("{:.4}", bal.total_spent_htc).normal());
    println!();
...
async fn deposit(ctx: &ClientContext, amount: f64, method: &str) -> Result<()> {
    ...
    println!("─────────────────────────────────────────");
    println!("  Amount:     {} HC", format!("{:.2}", dep.amount_htc).green().bold());
...
async fn withdraw(ctx: &ClientContext, address: &str, amount: f64) -> Result<()> {
    ...
    println!("  Amount:   {} HC", format!("{:.4}", w.amount_htc).green());
...
async fn show_history(ctx: &ClientContext, limit: u32, kind: Option<String>) -> Result<()> {
    ...
    for tx in &result.transactions {
        let amount_str = if tx.amount_htc >= 0.0 {
            format!("+{:.4}", tx.amount_htc).green().to_string()
        } else {
            format!("{:.4}", tx.amount_htc).red().to_string()
        };
