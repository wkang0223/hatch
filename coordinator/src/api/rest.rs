...
fn runtime_int_to_str(rt: i32) -> &'static str {
    match rt {
        0  => "mlx",
        1  => "torch-mps",
        2  => "onnx-coreml",
...
async fn health() -> Json<serde_json::Value> {
    Json(serde_json::json!({ "status": "ok", "service": "hatch-coordinator" }))
}
...
async fn list_providers(
    ...
    let providers: Vec<_> = rows.into_iter().map(|r| serde_json::json!({
        ...
        "floor_price_htc_per_hour": r.floor_price_htc_per_hour,
        "region":                   r.region,
...
async fn get_provider(
    ...
    Ok(Json(serde_json::json!({
        ...
        "state": row.state,
        "floor_price_htc_per_hour": row.floor_price_htc_per_hour,
...
async fn list_jobs_rest(
    ...
    let jobs: Vec<_> = rows.into_iter().map(|r| {
        // Derive actual_cost_htc from runtime (92% of gross)
        let actual_cost_htc = match (r.price_per_hour, r.actual_runtime_s) {
            (Some(p), Some(s)) => Some(p * (s as f64 / 3600.0)),
            _ => None,
        };
        // Convert integer runtime enum back to string name for CLI
        let runtime_str = runtime_int_to_str(r.runtime.unwrap_or(4));
        serde_json::json!({
            "id":                 r.job_id,
            "job_id":             r.job_id,
            ...
            "actual_cost_htc":    actual_cost_htc,
...
async fn submit_job(
    ...
    let db_err = || (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "db_error"})));

    if body.max_duration_secs <= 0 {
        return Err((StatusCode::BAD_REQUEST, Json(serde_json::json!({
            "ok": false, "error": "invalid_duration",
            "message": "max_duration_secs must be > 0",
        }))));
    }
    if body.max_price_per_hour <= 0.0 {
        return Err((StatusCode::BAD_REQUEST, Json(serde_json::json!({
            "ok": false, "error": "invalid_price",
            "message": "max_price_per_hour must be > 0",
        }))));
    }
    ...
    // Single DB transaction: if any step fails (job INSERT, escrow INSERT, etc.)
    // the escrow deduction is automatically rolled back — no partial state possible.
    let mut tx = state.db.begin().await.map_err(|_| db_err())?;

    // Atomic check-and-escrow inside the transaction.
    let rows = sqlx::query!(
        r#"
        UPDATE credit_accounts
        SET available_htc = available_htc - $1,
            escrowed_htc  = escrowed_htc  + $1,
            updated_at    = now()
        WHERE account_id    = $2
          AND available_htc >= $1
        "#,
    ...
    if rows.rows_affected() == 0 {
        // Transaction rolls back automatically when tx is dropped.
        let available: f64 = sqlx::query_scalar(
            "SELECT COALESCE(available_htc, 0.0) FROM credit_accounts WHERE account_id = $1"
        )
        ...
        return Err((StatusCode::PAYMENT_REQUIRED, Json(serde_json::json!({
            "ok": false,
            "error": "insufficient_balance",
            "message": format!(
                "Need {:.4} NMC escrow for max job cost, only {:.4} available. \
    ...
    sqlx::query!(
        r#"
        INSERT INTO escrows (escrow_id, job_id, consumer_id, locked_htc)
        VALUES ($1, $2, $3, $4)
        "#,
        escrow_id,
        job_id,
    ...
    sqlx::query!(
        r#"
        INSERT INTO transactions (tx_id, account_id, tx_type, amount_htc, balance_after, reference, description)
        VALUES ($1, $2, 'escrow_lock', $3,
            (SELECT available_htc FROM credit_accounts WHERE account_id = $2),
            $4, 'Job escrow locked')
        "#,
        tx_id,
        body.account_id,
...
async fn cancel_job(
    ...
    // Fetch the locked amount once so we only read it once (avoids the
    // two-subquery race in the previous UPDATE … WHERE … SELECT … SELECT …).
    let locked_htc: f64 = sqlx::query!(
        "SELECT locked_htc FROM escrows WHERE job_id = $1 AND state = 'locked' LIMIT 1",
        job_id,
    )
    .fetch_optional(&state.db)
    .await
    ...
    if locked_htc > 0.0 {
        sqlx::query!(
            ...
            UPDATE credit_accounts
            SET available_htc = available_htc + $1,
                escrowed_htc  = GREATEST(0.0, escrowed_htc - $1),
                updated_at    = now()
            WHERE account_id  = $2
            "#,
            locked_htc,
        ...
        // Audit trail — record the refund so wallet history shows it
        let tx_id = Uuid::new_v4().to_string();
        sqlx::query!(
            r#"INSERT INTO transactions
                (tx_id, account_id, tx_type, amount_htc, reference, description)
               VALUES ($1, $2, 'escrow_release', $3, $4, 'Job cancelled — escrow refunded')"#,
            tx_id, job.consumer_id, locked_htc, job_id,
        )
        .execute(&state.db)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    info!(job_id = %job_id, refund_htc = locked_htc, "Job cancelled");
    Ok(Json(serde_json::json!({ "ok": true, "job_id": job_id, "state": "cancelled", "refund_htc": locked_htc })))
}
...
async fn get_balance(
    ...
    let bal = sqlx::query!(
        r#"
        SELECT available_htc, escrowed_htc, total_earned_htc, total_spent_htc
    ...
    match bal {
        Some(b) => Ok(Json(serde_json::json!({
            ...
            "available_htc":    b.available_htc,
            "escrowed_htc":     b.escrowed_htc,
            "total_earned_htc": b.total_earned_htc,
            "total_spent_htc":  b.total_spent_htc,
        }))),
        None => Ok(Json(serde_json::json!({
            "account_id":       account_id,
            "available_htc":    0.0,
            "escrowed_htc":     0.0,
            "total_earned_htc": 0.0,
            "total_spent_htc":  0.0,
        }))),
    }
}
...
async fn list_transactions(
    ...
    let rows = sqlx::query!(
        r#"
        SELECT tx_id, tx_type, amount_htc, reference, description, created_at
        FROM transactions
        WHERE account_id = $1
        ORDER BY created_at DESC
        LIMIT $2
...
async fn get_account(
    ...
    let bal = sqlx::query!(
        "SELECT available_htc, escrowed_htc, total_earned_htc, total_spent_htc FROM credit_accounts WHERE account_id = $1",
        account_id,
    )
    .fetch_optional(&state.db)
    .await
...
async fn complete_job(
    ...
    let gross_htc = job.price_per_hour.unwrap_or(0.05) * hours;
    let provider_htc = gross_htc * (1.0 - PLATFORM_FEE); // 92% to provider
    let platform_htc = gross_htc * PLATFORM_FEE;          // 8%  to platform
    ...
    sqlx::query!(
        r#"
        INSERT INTO credit_accounts (account_id, available_htc, total_earned_htc)
        VALUES ($1, $2, $2)
        ON CONFLICT (account_id) DO UPDATE
        SET available_htc    = credit_accounts.available_htc    + $2,
            total_earned_htc = credit_accounts.total_earned_htc + $2,
            updated_at       = now()
        "#,
        body.provider_id, provider_htc,
    )
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    // Fetch the locked escrow amount using a plain row query (avoids COALESCE type ambiguity).
    let locked_htc: f64 = sqlx::query!(
        "SELECT locked_htc FROM escrows WHERE job_id = $1 AND state = 'locked' LIMIT 1",
        job_id,
    )
    ...
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
    .map(|row| row.locked_htc)
    .unwrap_or(0.0);

    let refund_htc = (locked_htc - gross_htc).max(0.0);

    // Release consumer escrow atomically in SQL:
    //   • Remove the locked amount from escrowed_htc
    //   • Add back the unused portion (refund) to available_htc
    //   • Charge actual cost to total_spent_htc
    sqlx::query!(
        r#"
        UPDATE credit_accounts
        SET escrowed_htc    = GREATEST(0.0, escrowed_htc - $1),
            available_htc   = available_htc + $2,
            total_spent_htc = total_spent_htc + $3,
            updated_at      = now()
        WHERE account_id   = $4
        "#,
        locked_htc, refund_htc, gross_htc, job.consumer_id,
    )
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    // Mark escrow row as released
    sqlx::query!(
        "UPDATE escrows SET state = 'released', settled_at = now() WHERE job_id = $1 AND state = 'locked'",
        job_id,
    )
    ...
    // Free provider — guard on state = 'leased' prevents double-increment on retry.
    sqlx::query!(
        r#"UPDATE providers
           SET state = 'available', active_job_id = NULL,
               jobs_completed = jobs_completed + 1
           WHERE provider_id = $1 AND state = 'leased'"#,
        body.provider_id,
    )
    .execute(&state.db)
    .await
    ...
    // Log provider earn transaction
    let tx_id = Uuid::new_v4().to_string();
    sqlx::query!(
        r#"
        INSERT INTO transactions (tx_id, account_id, tx_type, amount_htc, reference, description)
        VALUES ($1, $2, 'earn', $3, $4, 'Job completion credit')
        "#,
        tx_id, body.provider_id, provider_htc, job_id,
    )
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    // Log consumer escrow_release transaction (refund of unused escrow)
    if refund_htc > 0.0 {
        let tx_id2 = Uuid::new_v4().to_string();
        sqlx::query!(
            r#"
            INSERT INTO transactions (tx_id, account_id, tx_type, amount_htc, reference, description)
            VALUES ($1, $2, 'escrow_release', $3, $4, 'Unused escrow refunded after job completion')
            "#,
            tx_id2, job.consumer_id, refund_htc, job_id,
        )
        .execute(&state.db)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    // The oracle is a no-op when HATCH_LEDGER_URL + INTERNAL_SECRET are absent.
    if state.oracle.enabled {
        let oracle   = std::sync::Arc::clone(&state.oracle);
        let jid      = job_id.clone();
        let cost_htc = gross_htc;
        tokio::spawn(async move {
            match oracle.release_escrow(&jid, cost_htc).await {
    ...
    Ok(Json(serde_json::json!({
        "ok":            true,
        "gross_htc":     gross_htc,
        "locked_htc":    locked_htc,
        "refund_htc":    refund_htc,
        "provider_htc":  provider_htc,
        "platform_htc":  platform_htc,
        "on_chain":      state.oracle.enabled,
    })))
}
...
async fn fail_job(
    ...
    // Verify caller is the assigned provider — prevents a rogue provider from
    // failing another provider's job and triggering an unwarranted escrow refund.
    let job = sqlx::query!(
        "SELECT consumer_id FROM jobs WHERE job_id = $1 AND provider_id = $2",
        job_id,
        body.provider_id,
    )
    .fetch_optional(&state.db)
    ...
    sqlx::query!(
        "UPDATE jobs SET state = 'failed', failure_reason = $1, completed_at = now() WHERE job_id = $2",
        body.reason,
        job_id,
    )
    .execute(&state.db)
    ...
    // Fetch the amount locked for THIS specific job — never touch other jobs' escrow.
    let locked_htc: f64 = sqlx::query!(
        "SELECT locked_htc FROM escrows WHERE job_id = $1 AND state = 'locked' LIMIT 1",
        job_id,
    )
    ...
    if locked_htc > 0.0 {
        // Refund only this job's locked amount — not all escrowed_htc.
        sqlx::query!(
            r#"
            UPDATE credit_accounts
            SET available_htc = available_htc + $1,
        ...
        // Audit trail — record the refund in wallet history
        let tx_id = Uuid::new_v4().to_string();
        sqlx::query!(
            r#"INSERT INTO transactions
                (tx_id, account_id, tx_type, amount_htc, reference, description)
               VALUES ($1, $2, 'escrow_release', $3, $4, 'Job failed — escrow refunded')"#,
            tx_id, job.consumer_id, locked_htc, job_id,
        )
        .execute(&state.db)
        .await
...
async fn stripe_credit(
    ...
    sqlx::query!(
        r#"
        INSERT INTO credit_accounts (account_id, available_htc, updated_at)
        VALUES ($1, $2, now())
        ON CONFLICT (account_id) DO UPDATE
        SET available_htc = credit_accounts.available_htc + $2,
            updated_at    = now()
        "#,
        body.account_id, body.amount_htc,
    )
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    sqlx::query!(
        ...
        "#,
        tx_id, body.account_id, body.amount_htc, body.stripe_session_id,
    )
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    info!(
        account_id = %body.account_id,
        amount_htc = body.amount_htc,
        session_id = %body.stripe_session_id,
        "Stripe credit applied"
    );

    Ok(Json(serde_json::json!({ "ok": true, "credited_htc": body.amount_htc })))
}
...
async fn withdraw(
    ...
    if !matches!(body.chain.as_str(), "arbitrum" | "solana" | "ethereum") {
        return Ok(Json(serde_json::json!({
            "ok": false,
            "error": "invalid_chain",
            "message": "chain must be 'arbitrum', 'solana', or 'ethereum'",
        })));
    }
    ...
    if bal < body.amount_htc {
        return Ok(Json(serde_json::json!({
            "ok":    false,
            "error": "insufficient_balance",
            "message": format!("Available: {:.4} NMC, requested: {:.4}", bal, body.amount_htc),
        })));
    }

    // Deduct from balance — runtime query, no offline cache needed
    sqlx::query(
        "UPDATE credit_accounts SET available_htc = available_htc - $1, updated_at = now() WHERE account_id = $2"
    )
    .bind(body.amount_htc)
    .bind(&body.account_id)
    .execute(&state.db)
    .await
    .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let tx_id = Uuid::new_v4().to_string();
    sqlx::query(
        "INSERT INTO transactions (tx_id, account_id, tx_type, amount_htc, reference, description) VALUES ($1, $2, 'withdrawal', $3, $4, 'Withdrawal request queued')"
    )
    .bind(&tx_id)
    .bind(&body.account_id)
    .bind(-body.amount_htc)
