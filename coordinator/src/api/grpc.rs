...
impl AgentService for AgentGrpcService {
    async fn register_provider(
        ...
        match sqlx::query!(
            ...
            INSERT INTO providers (
                ...
                floor_price_htc_per_hour, state, nm_version
            ...
            ON CONFLICT (provider_id) DO UPDATE SET
                ...
                floor_price_htc_per_hour = EXCLUDED.floor_price_htc_per_hour,
                state = 'offline',
                updated_at = now()
            "#,
            provider_id,
    ...
    async fn report_completion(
        &self,
        request: Request<hatch_proto::agent::JobComplete>,
    ) -> Result<Response<JobCompleteAck>, Status> {
        ...
        if job_state == "complete" && !jc.provider_id.is_empty() {
            ...
            if let Some(row) = updated {
                ...
                for milestone_num in (claimed + 1)..=(milestones_earned) {
                    ...
                    let ledger_url = std::env::var("HATCH_LEDGER_URL")
...
impl ConsumerService for ConsumerGrpcService {
    async fn submit_job(
        ...
        if let Some(ref nc) = self.state.nats {
            ...
            let _ = nc.publish("hatch.jobs.new", payload.into_bytes().into()).await;
        }
    ...
    async fn list_providers(
        ...
        let rows = sqlx::query!(
            ...
            WHERE state = 'available'
              ...
              AND ($2 = '' OR region = $2)
              AND (floor_price_htc_per_hour <= $3 OR $3 = 0)
        ...
        let providers: Vec<hatch_proto::job::ProviderListing> = rows.into_iter().map(|r| {
            hatch_proto::job::ProviderListing {
                ...
                price_per_hour: r.floor_price_htc_per_hour.unwrap_or(0.0),
