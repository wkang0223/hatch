...
async fn run_auction_cycle(db: &PgPool, nats: Option<&async_nats::Client>) -> Result<()> {
    ...
    for job in queued_jobs {
        ...
        // Map job runtime integer to the string used in providers.installed_runtimes
        let required_runtime = runtime_int_to_str(job.runtime.unwrap_or(4));
...
async fn find_best_provider(
    ...
    // installed_runtimes is a TEXT[] column; @> checks if the array contains the value.
    // When required_runtime is "shell" any provider qualifies (shell is always available).
    let candidates = sqlx::query!(
        r#"
        SELECT provider_id, floor_price_htc_per_hour, trust_score, jobs_completed, region
        FROM providers
        WHERE state = 'available'
          AND max_job_ram_gb >= $1
          AND floor_price_htc_per_hour <= $2
          AND (
              $3 = 'shell'
              OR installed_runtimes @> ARRAY[$3]
          )
    ...
    let best = candidates.into_iter().max_by(|a, b| {
        let score_a = simple_score(
            a.floor_price_htc_per_hour.unwrap_or(0.1),
        ...
        let score_b = simple_score(
            b.floor_price_htc_per_hour.unwrap_or(0.1),
...
fn simple_score(
    ...
    // 10-point bonus for exact region match — enough to prefer same-region providers
    // at similar price/trust, but not enough to override a drastically cheaper one.
    let region_bonus = match (provider_region, preferred_region) {
        (Some(p), Some(r)) if p == r => 0.10,
        _ => 0.0,
    };
    0.55 * price_norm + 0.35 * trust_norm + region_bonus
}
...
async fn assign_job(
    ...
    if let Some(nc) = nats {
        ...
        let _ = nc
            .publish(
                format!("hatch.provider.{}", provider_id),
...
/// Map the integer runtime enum stored in `jobs.runtime` back to its string key.
/// These keys must match what agents register in `providers.installed_runtimes`.
fn runtime_int_to_str(runtime: i32) -> &'static str {
    match runtime {
        0  => "mlx",
        1  => "torch-mps",
        2  => "onnx-coreml",
        3  => "llama-cpp",
        4  => "shell",
        // Linux runtimes use integers ≥ 10
        10 => "torch-cuda",
        11 => "onnx-cuda",
        12 => "tensorrt",
        13 => "llama-cpp-cuda",
...
/// Provider heartbeat timeout — 120 s. Used in run_heartbeat_watcher log message
/// and the INTERVAL '120 seconds' literal in check_stale_providers.
...
async fn check_stale_providers(db: &PgPool) -> Result<()> {
    ...
    // Use a literal interval — the string-interpolation approach ($1 || ' seconds')
    // is fragile and bypasses query caching.
...
async fn refund_consumer_escrow(db: &PgPool, job_id: &str) -> Result<()> {
    ...
    let escrow = sqlx::query!(
        "SELECT escrow_id, consumer_id, locked_htc FROM escrows WHERE job_id = $1 AND state = 'locked'",
        job_id,
    )
    .fetch_optional(db)
    .await?;
    ...
    sqlx::query!(
        r#"INSERT INTO credit_accounts (account_id, available_htc)
           VALUES ($1, $2)
           ON CONFLICT (account_id) DO UPDATE
             SET available_htc = credit_accounts.available_htc + $2,
                 escrowed_htc  = GREATEST(0, credit_accounts.escrowed_htc - $2),
                 updated_at    = now()"#,
        escrow.consumer_id,
        escrow.locked_htc,
    )
    ...
    sqlx::query!(
        r#"INSERT INTO transactions (tx_id, account_id, tx_type, amount_htc, reference, description)
