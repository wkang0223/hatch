...
//!
//! If `HATCH_LEDGER_URL` is absent the calls are silently skipped —
//! Phase 1/2 deployments run without a ledger service.

use anyhow::Result;
use tracing::{info, warn};

/// HTTP client that forwards settlement requests to `hatch-ledger`.
///
...
impl SettlementOracle {
    ...
    /// - `HATCH_LEDGER_URL`   — base URL of the ledger service (e.g. `http://ledger:8082`)
    /// - `INTERNAL_SECRET` — shared secret for ledger→coordinator auth
    pub fn from_env() -> Self {
        let ledger_url = std::env::var("HATCH_LEDGER_URL").unwrap_or_default();
        if ledger_url.is_empty() {
            info!("HATCH_LEDGER_URL not set — on-chain settlement forwarding disabled");
