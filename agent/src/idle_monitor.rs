...
use anyhow::Result;
use hatch_common::{config::AgentConfig, MacChipInfo, Runtime};
use hatch_crypto::HatchKeypair;
use hatch_macos::idle::{IdleDetector, IdleState};
...
impl IdleMonitor {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        // 5s poll keeps job start latency low (auction cycle is 30s, so worst
        // case is 30s+5s = 35s from submission to execution start).
        let poll_interval      = Duration::from_secs(5);
        let mut last_heartbeat = Instant::now();

        // Detect Python interpreter once at startup on a blocking thread.
        // best_python_prefix() spawns subprocesses — calling it inline would
        // block the tokio event loop while it probes each python binary.
        let python_prefix = tokio::task::spawn_blocking(best_python_prefix)
            .await
            .unwrap_or_else(|_| "/opt/homebrew".to_string());

        // Channel for the job-runner task to signal completion/failure back to
        // the main loop so current_state can be reset to Busy.
        let job_done: Arc<Mutex<bool>> = Arc::new(Mutex::new(false));

        info!("Idle monitor started — polling every {}s", poll_interval.as_secs());

        loop {
            // ── Check if a running job finished (reset to Busy) ───────────
            {
                let mut done = job_done.lock().unwrap();
                if *done {
                    *done = false;
                    info!("Job finished — resetting state after job");

                    // The idle detector runs independently and may already be in
                    // Available state (it has no concept of "job running").  If so,
                    // no state transition will fire and announce_available() will
                    // never be called — leaving the coordinator stuck thinking we're
                    // busy.  Re-announce explicitly when the detector is already idle.
                    if matches!(detector.current_state(), IdleState::Available) {
                        current_state = IdleState::Available;
                        info!("Detector already idle — re-announcing availability");
                        if let Err(e) = self.announce_available().await {
                            warn!(error = %e, "Failed to re-announce availability after job");
                        }
                    } else {
                        current_state = IdleState::Busy;
                        info!("Resetting to Busy state");
                    }
                }
            }
            ...
            if matches!(current_state, IdleState::Available) {
                match self.poll_for_job().await {
                    Ok(Some(job)) => {
                        ...
                        // job_done_tx lets us reset current_state when the job ends.
                        let job_clone        = job.clone();
                        let coord            = self.coordinator.clone();
                        let provider_id      = self.coordinator.provider_id().to_string();
                        let python_prefix    = python_prefix.clone();
                        let job_done_signal  = Arc::clone(&job_done);
                        // Shared client — reused for all HTTP calls in this job's lifetime.
                        ...
                        tokio::spawn(async move {
                            ...
                            // Per-job heartbeat task — uses the shared client.
                            let hb_http   = std::sync::Arc::clone(&http);
                            ...
                            let heartbeat_task = tokio::spawn(async move {
                                ...
                                loop {
                                    ...
                                    let gpu = hatch_macos::gpu_detect::sample_gpu_utilization();
                                    ...
                                    let _ = hb_http.post(&url).json(&body).send().await;
                                }
                            });
                            ...
                            match JobRunner::run(&job_clone, &python_prefix, &rest_base, &provider_id).await {
                                Ok(result) => {
                                    ...
                                    if result.exit_code == 0 {
                                        report_completion(&coord, &provider_id, result, &http).await;
                                    } else {
                                        // Non-zero exit: upload any captured output before reporting failure.
                                        let exit_code = result.exit_code;
                                        if !result.output_log.is_empty() {
                                            let url = format!("{}/api/v1/jobs/{}/logs", rest_base, job_id_str);
                                            let _ = http.post(&url)
                                                .json(&serde_json::json!({ "output": result.output_log }))
                                                .send().await;
                                        }
                                        report_failure(&coord, &provider_id, &job_id_str,
                                            &format!("exit code {}", exit_code), true).await;
                                    }
                                }
                                Err(e) => {
                                    // Rust-level failure (spawn error, I/O error, etc.)
                                    heartbeat_task.abort();
                            ...
                            // Signal main loop that the job is done
                            *job_done_signal.lock().unwrap() = true;
                        });
                    }
                    Ok(None) => {} // No job yet
            ...
            if last_heartbeat.elapsed() >= heartbeat_interval {
                ...
                let gpu_stats = hatch_macos::gpu_detect::sample_gpu_utilization();
    ...
    async fn poll_for_job(&self) -> Result<Option<JobSpec>> {
        ...
        let runtime = hatch_common::Runtime::from_str(runtime_str)
...
pub struct JobSpec {
    ...
    pub runtime: hatch_common::Runtime,
    pub min_ram_gb: u32,
...
async fn report_completion(
    ...
    // 1. Upload stdout+stderr log so consumers can read it via `hatch job logs`.
    if !result.output_log.is_empty() {
        let log_url  = format!("{}/api/v1/jobs/{}/logs", base, job_id);
        let log_body = serde_json::json!({ "output": result.output_log });
        if let Err(e) = client.post(&log_url).json(&log_body).send().await {
            warn!(job_id, error = %e, "Failed to upload job log (non-fatal)");
        }
    }

    // 2. Mark job complete + trigger escrow settlement.
    let url  = format!("{}/api/v1/jobs/{}/complete", base, job_id);
    let body = serde_json::json!({
    ...
    match client.post(&url).json(&body).send().await {
        Ok(r) if r.status().is_success() => info!(job_id, "Completion reported, credits credited"),
        Ok(r) => error!(job_id, status = %r.status(), "Completion report rejected"),
        Err(e) => error!(job_id, error = %e, "Completion report failed"),
    }
}
