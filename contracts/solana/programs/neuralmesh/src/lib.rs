//! Hatch on-chain programme — Solana / Anchor 0.30
//!
//! Instructions:
...
pub const HTC_DECIMALS: u8 = 9;
pub const HTC_NAME: &str = "Hatch Token Credit";
pub const HTC_SYMBOL: &str = "HTC";
...
pub mod hatch {
    ...
    pub fn initialize(
        ...
        require!(platform_fee_bps <= MAX_FEE_BPS, HatchError::FeeTooHigh);
        let cfg = &mut ctx.accounts.config;
        cfg.authority = ctx.accounts.authority.key();
        cfg.htc_mint = ctx.accounts.htc_mint.key();
    ...
    pub fn bridge_mint(
        ...
        let cfg = &mut ctx.accounts.config;
        require!(amount <= cfg.bridge_mint_limit, HatchError::MintLimitExceeded);

        // Check and advance nonce
        let nonce_acc = &mut ctx.accounts.bridge_nonce;
        require!(nonce == nonce_acc.next_nonce, HatchError::Unauthorized);
        ...
        mint_to(
            CpiContext::new_with_signer(
                ...
                MintTo {
                    mint: ctx.accounts.htc_mint.to_account_info(),
    ...
    pub fn register_provider(
        ...
        require!(gpu_vendor.len() <= 32, HatchError::VendorTooLong);
        require!(gpu_model.len() <= 64, HatchError::ModelTooLong);
        require!(!ctx.accounts.provider_record.revoked, HatchError::ProviderRevoked);
    ...
    pub fn create_escrow(
        ...
        require!(job_id.len() <= 64, HatchError::JobIdTooLong);
    ...
    pub fn lock_escrow(ctx: Context<LockEscrow>) -> Result<()> {
        let esc = &mut ctx.accounts.escrow;
        require!(esc.state == EscrowState::Open, HatchError::EscrowNotOpen);
    ...
    /// `actual_cost` ≤ locked_htc; remainder is refunded to consumer.
    pub fn release_escrow(
        ctx: Context<ReleaseEscrow>,
        actual_cost: u64,
    ) -> Result<()> {
        let esc = &mut ctx.accounts.escrow;
        require!(esc.state == EscrowState::Locked, HatchError::EscrowNotLocked);
        require!(actual_cost <= esc.locked_htc, HatchError::CostExceedsLocked);
        ...
        let consumer_refund = esc.locked_htc.checked_sub(actual_cost).unwrap();
    ...
    pub fn update_config(
        ...
        if let Some(fee) = platform_fee_bps {
            require!(fee <= MAX_FEE_BPS, HatchError::FeeTooHigh);
...
pub struct Initialize<'info> {
    ...
    pub htc_mint: Account<'info, Mint>,
...
pub struct BridgeMint<'info> {
    #[account(
        ...
        seeds = [b"config"],
        bump = config.bump,
        has_one = authority @ HatchError::Unauthorized,
    )]
    pub config: Account<'info, ProgramConfig>,
    #[account(mut, address = config.htc_mint)]
    pub htc_mint: Account<'info, Mint>,
    #[account(mut)]
...
pub struct BridgeBurn<'info> {
    ...
    #[account(mut, address = config.htc_mint)]
    pub htc_mint: Account<'info, Mint>,
    #[account(mut, token::mint = htc_mint, token::authority = user)]
...
pub struct CreateEscrow<'info> {
    ...
    #[account(mut, token::mint = config.htc_mint, token::authority = consumer)]
    ...
    #[account(address = config.htc_mint)]
    pub htc_mint: Account<'info, Mint>,
    pub token_program: Program<'info, Token>,
    pub system_program: Program<'info, System>,
    pub rent: Sysvar<'info, Rent>,
...
pub struct LockEscrow<'info> {
    #[account(
        ...
        bump = escrow.bump,
        has_one = provider @ HatchError::Unauthorized,
    )]
    pub escrow: Account<'info, JobEscrow>,
    pub provider: Signer<'info>,
}
